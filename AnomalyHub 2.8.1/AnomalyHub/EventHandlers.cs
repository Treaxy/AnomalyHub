        ...
        public void Spawn(RespawningTeamEventArgs ev)
        {
            try
            {
                if (ev.NextKnownTeam == SpawnableTeamType.NineTailedFox)
                {
                    ...
                    {
                        blue = true;
                        Timing.RunCoroutine(light(false,false));
                    }
                }
                if (ev.NextKnownTeam == SpawnableTeamType.ChaosInsurgency)
                {
                    ...
                    {
                        blue = false;
                        Timing.RunCoroutine(light(true,false));
                    }
                ...
                {
                    if (Plugin.Instance.Config.UIUspawnLight && Plugin.Instance.Config.SHspawnLight)
                    { for (int i = 0; i < 3;){Log.Warn("UIU and SH lights is enabled! Some Bug's it couled be."); }}
                    ...
                    if (Plugin.Instance.Config.SHspawnLight)
                    {
                        blue = false;
                        red = true;
                        Timing.RunCoroutine(light(false, true));
                    }
                }
            }
            catch (Exception e)
